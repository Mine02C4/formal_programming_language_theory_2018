			...
			for (BiLink nodel = bb.instrList().first(); !nodel.atEnd(); nodel = nodel.next()) {
				...
				if (node.opCode == Op.SET) {
					if (node.kid(0).opCode == Op.REG && node.kid(1).opCode != Op.INTCONST
							&& node.kid(1).opCode != Op.FLOATCONST && node.kid(1).opCode != Op.REG
							&& node.kid(1).opCode != Op.CALL && node.kid(1).opCode != Op.USE
		...
		for (BiLink nodel = bb.instrList().first(); !nodel.atEnd(); nodel = nodel.next()) {
			...
			case Op.SET:
				...
				if (node.kid(0).opCode == Op.REG) {
					...
					if (node.kid(1).opCode != Op.INTCONST && node.kid(1).opCode != Op.FLOATCONST
